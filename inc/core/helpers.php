...
function cairo_get_page_link_url($i)
{
    global $wp_rewrite;
    $post = get_post();
    $query_args = array();
...
add_filter('wp_link_pages', 'cairo_link_page', null, 2);

function cairo_link_page($output, $args) {
    ...
    if($multipage) {
        ...
        $paging_text = sprintf(esc_html__('Page %s of %s', 'cairo'), $page, $numpages);

        $output =
            "<div class=\"codepages_pagelinks codepages_pagination\">
                <span class=\"page_info\">{$paging_text}</span>
                <div class=\"nav_link\">";


        if($page > 1) {
            $prev_url = cairo_get_page_link_url($page - 1);
            $output .= "<a class=\"page_nav prev\" href=\"{$prev_url}\"><span class=\"navtext\">" . esc_html__('Prev','cairo') . "</span></a>";
        }
        ...
        {
            if ( $n == $page ) {
                $output .= "<a class=\"page_number active\">{$n}</a>";
            } else
            {
                if ( $n <= $end_size || ( $page && $n >= $page - $mid_size && $n <= $page + $mid_size ) || $n > $numpages - $end_size )
                {
                    $loop_url = cairo_get_page_link_url($n);
        ...
        if($page < $numpages) {
            $next_url = cairo_get_page_link_url($page + 1);
            $output .= "<a class=\"page_nav next\" href=\"{$next_url}\"><span class=\"navtext\">" . esc_html__('Next','cairo') . "</span></a>";
        }
